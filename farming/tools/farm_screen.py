...
v4 изменения против v3 (пересмотр целевой функции + Gamma Phase A):
  - [МЕТРИКА] Сортировка по our_daily (доход НАШИХ our_size шер), НЕ по
    ...
    our_pts/(comp_pts+our_pts), our_daily = our_share × pool.
    comp_pts = paired + excess/3 (модель Polymarket Qmin); upper-bound —
    uptime и наши собственные ордера в книге активных рынков не учтены.
...
"""
import json, time, urllib.request, urllib.error, urllib.parse, re, sys, os, uuid
...
def get_db_connection():
    """Create DB connection from DATABASE_URL env."""
    url = os.environ.get(
        "DATABASE_URL",
...
def insert_candidates_to_db(candidates, scan_run_id, conn):
    """Insert farm_screen candidates into farming_market_candidates.
    Returns (inserted_count, errors_count).
    """
    if not candidates:
        return 0, 0
    sql = """
        INSERT INTO farming_market_candidates (
            gamma_id, condition_id, clob_token_ids, question, slug, category,
            rewards_min_size, rewards_max_spread,
            mid, best_bid, best_ask, liquidity_clob, volume_24hr_clob,
    ...
    for c in candidates:
        try:
            required_cap = c.get("required_capital") or Decimal("0")
            our_daily = c.get("our_daily") or Decimal("0")
            # est_daily_yield_pct = our_daily / required_capital (fraction)
            if required_cap > 0:
                est_yield = our_daily / required_cap
            else:
                est_yield = None
            # clob_token_ids: JSON array
            tok = c.get("token", "")
            clob_ids = json.dumps([tok]) if tok else None
            records = [
                c.get("gamma_id"),          # gamma_id
                c.get("condition_id"),       # condition_id
                clob_ids,                   # clob_token_ids (JSON)
                c.get("q"),                  # question
                c.get("slug"),              # slug
                c.get("group"),             # category ← group
                c.get("min_size", 0),       # rewards_min_size
                c.get("max_spread"),        # rewards_max_spread
                c.get("mid"),               # mid
                c.get("best_bid"),          # best_ask (live from CLOB)
                c.get("best_ask"),          # best_ask (live from CLOB)
                c.get("pool", 0),           # liquidity_clob ← pool
                c.get("volume_24hr_clob"),  # volume_24hr_clob
                c.get("end"),               # end_date (date str)
                c.get("days_to_end"),       # days_to_end
                required_cap,               # required_capital
                4,                          # funnel_stage (прошли все гейты)
                our_daily,                  # our_daily_usd
                est_yield,                  # est_daily_yield_pct
                c.get("fees_enabled"),      # fees_enabled
                c.get("neg_risk"),          # neg_risk
                c.get("tick"),              # tick
            ...
            errors += 1
            print(f"[DB INSERT ERROR] {c.get('q','?')[:40]}: {e}")
    conn.commit()
    return inserted, errors


def run_retention_cleanup(conn, retention_days=30):
    """Delete records older than retention_days."""
    with conn.cursor() as cur:
        cur.execute("""
            DELETE FROM farming_market_candidates
...
for m in gm:
    ...
    # required_capital = rewardsMinSize × mid × 2 (capital for both legs)
    req_cap = float(m.get("rewardsMinSize") or 0) * mid * 2
    # days_to_end
    try:
        end_dt = datetime.fromisoformat(end.replace("Z", "+00:00"))
        dte = (end_dt.date() - TODAY.date()).days if end_dt.tzinfo else (end_dt.date() - TODAY.date()).days
    except Exception:
        dte = None
    cand.append({"gamma_id": m.get("id"),          # Gamma market id (PK)
                 ...
                 "end": end[:10],
                 # FARM-022: additional fields for DB
...
for c in cand:
    try:
            ...
            for lvl in b.get(side, []):
                ...
                if s < ms:
                    ...
                    if side == "bids":
                        ...
                        ask_pts += pts
        # comp_pts = paired + excess/3 (модель Polymarket Qmin)
        ...
        if comp_pts <= 0:
            ...
            our_share = our_pts / (comp_pts + our_pts)
            c["dead_book"] = False

        c["mid"] = round(mid, 3)
        c["pts_k"] = round(comp_pts / 1000, 1)
        ...
        c["usd_per_kpts"] = round(c["pool"] / (comp_pts / 1000), 2) if comp_pts > 0 else None
...
# ─────────────────────── FARM-022: запись в БД ───────────────────────
# Пишем ТОЛЬКО если DB доступен (ad-hoc режим по умолчанию не трогаем)
db_write_enabled = os.environ.get("FARM_SCREEN_DB_WRITE", "false").lower() in ("1", "true", "yes")
if db_write_enabled and HAS_PG:
    try:
        scan_run_id = generate_scan_run_id()
        conn = get_db_connection()
        inserted, errors = insert_candidates_to_db(cand, scan_run_id, conn)
        deleted = run_retention_cleanup(conn, retention_days=30)
        conn.close()
        print(f"\n[FARM-022 DB] scan_run_id={scan_run_id}, inserted={inserted}, errors={errors}, retention_deleted={deleted}")
    except Exception as e:
        print(f"\n[FARM-022 DB ERROR] {e}")
else:
    if not db_write_enabled:
        print("\n[FARM-022] DB write disabled (FARM_SCREEN_DB_WRITE=false). "
