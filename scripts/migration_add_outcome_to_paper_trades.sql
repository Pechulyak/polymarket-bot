-- Migration: Add outcome column to paper_trades insert in trigger
-- This copies the outcome from whale_trades to paper_trades when copying whale trades
...
BEGIN
    ...
    DROP FUNCTION IF EXISTS copy_whale_trade_to_paper();
...
-- Recreate the function with outcome column
CREATE OR REPLACE FUNCTION copy_whale_trade_to_paper() RETURNS TRIGGER AS $$
DECLARE
...
DO $$
BEGIN
    RAISE NOTICE 'Migration completed: outcome column will be copied from whale_trades to paper_trades';
END
$$;
