...
**Магистральный следующий шаг — 3A `roundtrip_open`.** Контейнер `roundtrip_builder` асинхронно (каждые 2 часа, sleep-loop в отдельном docker-сервисе) сканирует все BUY-сделки в `whale_trades`, агрегирует их по тройке `(wallet_address, market_id, outcome)` и для каждой ранее не виденной тройки создаёт OPEN-roundtrip в `whale_trade_roundtrips`. Связь между шагом 2B и 3A — **асинхронная через таблицу `whale_trades`**: коммит шага 2B делает строку видимой для следующей итерации `roundtrip_builder`, без прямого вызова. Задержка между записью сделки и её появлением в OPEN-roundtrip — до 2 часов (период цикла) плюс время фактического создания записи в `whale_trade_roundtrips`. Описание шага 3A — отдельный документ.

### Side-route: paper-ветка (sidebar)

Коммит шага 2B (блок 11) **синхронно** активирует DB-trigger `trigger_copy_whale_trade` в той же транзакции (по PROJECT_STATE: `whale_trades → paper_trades, status: OK`). Для китов с `copy_status='paper'` trigger создаёт **новую сущность** — запись в `paper_trades` с расчётом proportional Kelly sizing. Для всех остальных китов trigger ничего не делает.

Создание `paper_trades` — это **не шаг магистрали**, а параллельная side-route: рождается новый объект (paper-сделка), state самой сделки кита в `whale_trades` при этом не изменяется. Логически paper-ветка относится к sidebar-нумерации P1, P2, ... — отдельной от магистрали 1 → 2A/2B → 3A → 3B/3C → 4. Описание paper-ветки отложено до стабилизации магистрали (см. INDEX §3 «Paper-ветка»).
