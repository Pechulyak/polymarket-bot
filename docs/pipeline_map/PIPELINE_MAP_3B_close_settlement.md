...
- UPDATE полей `close_trade_id`, `close_side` — они остаются NULL после settlement (нет реального close trade).
...
| 9 | `close_size_usd` | размер закрываемой позиции в USD | копируется из `open_size_usd` в момент settlement UPDATE (для SETTLEMENT close_type нет close-trade с собственным размером) |

### 9.4 Что НЕ изменяется

Поля, явно остающиеся в исходном состоянии после UPDATE 3B:
- `close_trade_id`, `close_side` — остаются NULL (нет реального trade при SETTLEMENT)
...
- Поля close-через-SELL (`close_trade_id`, `close_side`, `matching_method`, `matching_confidence`) остаются NULL. `close_size_usd` копируется из `open_size_usd` в момент UPDATE.
...
Что НЕ обновляется на 3B:
  - close_trade_id, close_side                     (поля для 3C SELL-close)
