...
class LiquiditySkewStrategy:
    ...
    def detect_opportunity(
        ...
        large_trades = [
            ...
            if t['size'] >= self.large_order_threshold
        ]
...
class CrossMarketArbitrage:
    ...
    def detect_opportunity(
        ...
        # Calculate fees
        fees_bps = self.calculate_total_fees_bps(
            trade_size, 
...
class OrderBookImbalance:
    ...
    def detect_opportunity(
        ...
        if imbalance < self.imbalance_threshold and imbalance > (1 / self.imbalance_threshold):
            return None
        ...
        if imbalance > self.imbalance_threshold:
            ...
            side = 'buy'
            confidence = min(Decimal("0.58") + (imbalance - 2) * Decimal("0.02"), Decimal("0.65"))
        else:
            # More asks than bids = bearish
            side = 'sell'
            confidence = min(Decimal("0.58") + (1/imbalance - 2) * Decimal("0.02"), Decimal("0.65"))
