...
class WhaleDetector:
    ...
    def set_database(self, database_url: str) -> None:
        ...
        logger.info("whale_trades_repo_initialized")
    ...
    async def save_trade_to_db(
        ...
        Delegates to WhaleTradesRepo for validation, deduplication, and insert.
        Maintains original interface (returns bool) for backward compatibility.
        ...
        Returns:
            True if trade was saved, False if duplicate or rejected.
        """
        # Ensure repo is ready
        await self._ensure_database()
        if not self._whale_trades_repo:
            logger.warning("save_trade_no_repo")
            return False
        ...
        try:
            ...
            logger.error(
                ...
                error=str(e),
                trader=trader[:10] if trader else "unknown",
                market_id=market_id,
            )
            return False
    ...
    async def _paper_poll_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                # Log repo stats every paper poll cycle
