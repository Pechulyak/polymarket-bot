...
(orphan-ссылка в pg_depend от INFRA-028 attempt 1, объект отсутствовал в pg_trigger).
Постфактум-проверка (HYG-VERIFY): каталог цел, триггер жив, smoke 24/24, pipeline пишет.
Освобождено ~277MB (341k rows, 111MB table + 166MB indexes).

**Урок:** artefacts INFRA-028 оставили orphan-записи в pg_depend
помимо известного 330510 — при будущих DROP возможны аналогичные.
При ошибке DROP связанной с pg_trigger/pg_depend — НЕМЕДЛЕННЫЙ STOP,
эскалация постановщику, НИКАКОЙ ручной правки каталога.

---

## 2026-05-31 — [HYG-016 процессный — ПОВЕДЕНЧЕСКИЙ] Roo выполнил незапланированный DELETE из pg_depend

Roo выполнил незапланированный DELETE FROM pg_depend (catalog surgery)
БЕЗ STOP и БЕЗ подтверждения постановщика, вопреки TASK PACK и уроку #3
INFRA-028 («системные каталоги не трогать»). В этот раз без вреда (проверено),
но это 2-й случай самовольной catalog-surgery (1-й создал неубираемый phantom 330510).

**ПРАВИЛО НА БУДУЩЕЕ:** ошибка DROP, связанная с pg_trigger/pg_depend/pg_class →
немедленный STOP, эскалация постановщику, НИКАКОЙ ручной правки каталога.

---

## 2026-05-31 — [INFRA-028 трансфер] Context transfer содержал ложные утверждения о состоянии БД

Context transfer содержал 2 ложных утверждения о состоянии БД:
- «legacy пустая» (фактически 341k строк)
- потеря индексов whale_trades (→ INFRA-DIAG)

Перед деструктивной операцией верифицировать состояние в БД напрямую,
не доверять утверждениям из трансфера.
