...
1. `grep -r "УДАЛЁННОЕ_ИМЯ" src/` — проверить все удалённые/переименованные константы, функции, классы
2. Перечислить все модули, которые импортируют изменённый файл
3. Перечислить все контейнеры, которые используют изменённый код
4. Результат приложить к отчёту

### Правило миграций
Если код добавляет/удаляет поле в таблице — ALTER TABLE идёт в ТОМ ЖЕ коммите.
Если код убирает использование поля — проверить что поле не используется в ДРУГИХ модулях.
Нарушение = коммит не принимается.

### Правило модульных границ
whale_trade_roundtrips, paper_trades, trades — РАЗНЫЕ модули.
Не добавлять перекрёстные зависимости (foreign keys, поля из чужих таблиц) без явного согласования с пользователем.

### Post-deploy Verification
После каждого деплоя (docker compose build/restart):
1. Запустить `scripts/smoke_test.sh`
2. Дождаться результата (не менее 60 секунд наблюдения)
3. Приложить вывод smoke_test к отчёту
4. Если smoke_test FAIL — немедленный откат, не продолжать

## Правила для КРИТИЧЕСКИХ задач (ОБЯЗАТЕЛЬНО)

**Критическая задача** — любая задача, затрагивающая:
- Firewall / сетевые порты / iptables
- pg_hba.conf / аутентификация PostgreSQL
- Credentials / .env / пароли / ключи
- Docker volumes / persistent data
- docker-compose.yml (изменения, влияющие на БД или сеть)
- SSL / TLS / certificates
- Права доступа (chmod, chown, PostgreSQL GRANT/REVOKE на system tables)

Для таких задач действуют УСИЛЕННЫЕ правила поверх стандартного workflow:

### 1. Baseline до любых изменений
Перед первым изменением Roo ОБЯЗАН зафиксировать baseline:
- `bash scripts/smoke_test.sh` — полный вывод сохраняется
- Ручная проверка ключевых показателей (записи в БД за последние 5 минут, healthy контейнеры)
- Результат baseline — часть отчёта. Без baseline задача не начинается.

### 2. Двойная верификация каждого шага
Каждый ключевой шаг проверяется МИНИМУМ двумя независимыми способами:
- Пример: изменение pg_hba.conf → проверка 1: `docker exec psql "SHOW hba_file"`, проверка 2: реальное подключение с паролем и без
- Пример: открытие порта → проверка 1: `ss -tlnp`, проверка 2: подключение снаружи
- Один способ = недостаточно. Всегда два.

### 3. Обязательные стоп-точки
Roo ОБЯЗАН останавливаться после каждого ключевого шага и ждать явного review от STRATEGY:
- ❌ ЗАПРЕЩЕНО: "продолжаю следующий шаг" без подтверждения
- ❌ ЗАПРЕЩЕНО: самостоятельно принимать решения об успехе ("это не критично, продолжаю")
- ✅ ОБЯЗАТЕЛЬНО: "⏸ СТОП-ТОЧКА: жду review" + ждать
- Review делает STRATEGY (не Roo). Только STRATEGY может сказать "продолжай".

### 4. Никаких "pre-existing" отмазок
Если после изменения Roo появился новый FAIL или изменилось количество FAIL:
- ❌ ЗАПРЕЩЕНО: списывать на "pre-existing failure" без доказательств
- ✅ ОБЯЗАТЕЛЬНО: сравнить с baseline (Шаг 1)
- Если FAIL появился после изменений — это НАША поломка, разбираемся до конца
- "Контейнеры работают — это главное" НЕ является признаком успеха

### 5. Rollback подготовлен ДО изменений
Перед первым изменением Roo ОБЯЗАН:
- Создать бэкапы всех изменяемых файлов (с timestamp)
- Описать точную последовательность rollback команд
- Убедиться что rollback можно выполнить за < 1 минуту
- Показать бэкапы и rollback-план STRATEGY перед началом

### 6. Credentials никогда не выводятся
Абсолютное правило (после инцидента 11.03.2026):
- ❌ ЗАПРЕЩЕНО: `echo $PASSWORD`, `env | grep POSTGRES`, вывод содержимого .env
- ❌ ЗАПРЕЩЕНО: пароли в аргументах команд (видны в `ps aux`)
- ✅ РАЗРЕШЕНО: пароли через stdin (heredoc), через переменные окружения, через `.env` с permissions 600
- Нарушение = немедленная остановка задачи, фиксация в errors-log

### 7. Критические задачи закрывает ТОЛЬКО STRATEGY
- Roo не имеет права переводить задачу в READY/DONE
- Roo не имеет права говорить "задача выполнена"
- Только STRATEGY (пользователь) может подтвердить завершение после review всех шагов
- Формальное принятие отчёта Roo без review = нарушение правила

### 8. При любом сомнении — rollback
Если Roo видит:
- Неожиданный вывод команды
- FAIL в проверке, который не был предсказан
- Ошибку подключения
- Изменение поведения контейнера

→ немедленный стоп, показ ситуации STRATEGY, НЕ пытаться "починить самостоятельно".

### Verification секция в отчёте
Каждый коммит Roo содержит:
- Blast radius: какие модули затронуты
- Grep check: результат поиска по удалённым именам
- Smoke test: вывод скрипта (или ручная проверка с доказательством)
- Контейнеры: какие перезапущены, статус после рестарта
...
- Любое изменение firewall (ufw, iptables)
- Любое изменение pg_hba.conf
- Любое изменение connection strings (DATABASE_URL)
- Создание/удаление PostgreSQL users
- Изменение PostgreSQL GRANT/REVOKE на уровне БД
- Изменение docker-compose.yml в секциях ports/volumes/networks
- Генерация SSL сертификатов

## Верификация и диагностика

Все проверки, диагностика, аудит и верификация выполняются Roo как часть задачи.
Пользователь НЕ выполняет команды для проверки результата — это ответственность Roo.

Правила:
1. Каждый TASK PACK включает шаг верификации с конкретными командами и ожидаемым результатом
2. Roo выполняет все проверочные команды самостоятельно и показывает вывод
3. Ручные проверки пользователем допустимы только по его инициативе
4. Задача не может быть отмечена как READY/DONE без пройденной верификации
5. Если верификация требует ожидания (например, данные появятся через N минут) — Roo ждёт и проверяет повторно
